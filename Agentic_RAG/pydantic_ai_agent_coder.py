...
llm = os.getenv('PRIMARY_MODEL', 'gpt-4o')
...
async def retrieve_relevant_documentation(ctx: RunContext[PydanticAIDeps], user_query: str) -> str:
    """
    Retrieve relevant documentation chunk summaries based on the query with RAG.
    ...
    Returns:
        A formatted string containing the summaries of the top 5 most relevant documentation chunks
    """
    try:
...
async def get_page_content(ctx: RunContext[PydanticAIDeps], url: str) -> str:
    """
    Retrieve a summary of a specific documentation page by combining all its chunk summaries.
    ...
    Returns:
        str: The complete page summary with all chunk summaries combined in order
    """
    try:
        # Query Supabase for all chunks of this URL, ordered by chunk_number
        result = ctx.deps.supabase.from_('site_pages') \
            .select('title, summary, chunk_number') \
            .eq('url', url) \
            .eq('metadata->>source', 'pydantic_ai_docs') \
            .order('chunk_number') \
            .execute()
        ...
        # Format the page with its title and all chunk summaries
        page_title = result.data[0]['title'].split(' - ')[0]  # Get the main title
        formatted_content = [f"# {page_title}\n"]
        
        # Add each chunk's summary
        for chunk in result.data:
            formatted_content.append(f"## Chunk {chunk['chunk_number']} Summary\n{chunk['summary']}")
