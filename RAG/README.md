...
* **Technology:** Built with Streamlit, LangChain, and LangGraph. It uses a FAISS vector store and OpenAI models (`gpt-4o-mini`, `text-embedding-3-large`).
...
* **Core Functionality:** The agent can decide whether it needs to retrieve information from the source document (`COI.pdf`) to answer a question or if it can respond based on the conversation history alone.
* **Technology:** Uses LangGraph's `MessagesState` to manage memory, a tool-calling router to decide when to retrieve, and FAISS for the vector store.
...
* **Core Functionality:** Loads a PDF (`rag.pdf`), splits it into chunks, generates embeddings, indexes them in FAISS, and performs a similarity search to answer a query.
...
* **Core Functionality:** It uses `WebBaseLoader` to scrape content from a specified blog post, then indexes and queries the content using a simple LangGraph chain.
...
* **LLMs & Embeddings:** OpenAI (`gpt-4o-mini`, `gpt-3.5-turbo-instruct`, `text-embedding-3-large`) 
* **Vector Store:** FAISS (via `faiss-cpu`) 
* **Document Loading:** PyPDF, WebBaseLoader 
